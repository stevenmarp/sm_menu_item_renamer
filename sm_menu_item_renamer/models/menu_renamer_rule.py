...
class IrUiMenu(models.Model):
    ...
    def load_menus(self, debug):
        ...
        if mapping:
            # super() result is ormcached and shared: rebuild copies, never mutate
            menus = self._sm_rename_menu_tree(menus, mapping)
        return menus

    def _sm_rename_menu_tree(self, node, mapping):
        node = dict(node)
        if node.get("id") in mapping:
            node["name"] = mapping[node["id"]]
        if node.get("children"):
            node["children"] = [
                self._sm_rename_menu_tree(child, mapping) for child in node["children"]
            ]
        return node
